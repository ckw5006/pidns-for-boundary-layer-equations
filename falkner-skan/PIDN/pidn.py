...
def solve_falkner_skan_runge_kutta(x, beta, fpp_0):
    ...
    # 使用更稳定的积分方法，如 'Radau' 或 'BDF'，并严格控制误差精度
    sol = solve_ivp(equations, [x[0], x[-1]], y0, t_eval=x, method='Radau', rtol=1e-7, atol=1e-9)
...
beta_values = [-0.198, -0.19, -0.16, 0.0, 2.0]
fpp_0_values = [0.0, 0.086, 0.190, 0.4696, 1.6872]
beta_values = [2.0, 0.0, -0.16, -0.19, -0.198]
fpp_0_values = [1.6872, 0.4696, 0.190, 0.086, 0.0]
...
# 定义感兴趣的点
specific_x_full = np.linspace(0, 5, 500)
...
for beta, fpp_0, color in zip(beta_values, fpp_0_values, colors):
    ...
    function_values, grads1, x = train_model(model, criterion, optimizer, inputs, beta, fpp_0, epochs=3000,
                                             save_path=save_path)
...
plt.xlabel('x')
plt.ylabel('f(x)')
plt.title('Comparison of PIDNs and Runge-Kutta for Different Beta')
plt.legend()
...
#              label=f'True Derivative (Runge-Kutta) β={beta}', color=color, linestyle='-')
#
# plt.xlabel('x')
# plt.ylabel('f\'(x)')
# plt.title('Comparison of Predicted and True Derivative for Different Beta ')
# plt.legend()
