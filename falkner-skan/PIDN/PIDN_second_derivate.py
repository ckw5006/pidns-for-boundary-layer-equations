...
def compute_function_values_from_derivatives(grads1, inputs):
    delta_x = inputs[1, 0] - inputs[0, 0]  # 计算采样点之间的差值 (只考虑第一个维度)
    function_values = torch.cumsum(grads1, dim=0) * delta_x
    function_values = function_values.clone()  # 克隆张量以避免内存共享问题
    function_values -= function_values.clone()[0]  # 函数值从0点开始
...
# fpp_0_values = [0.0, 0.086, 0.190, 0.4696, 1.6872]

# beta_values = [-0.18, -0.19, 2.0]
...
# 定义感兴趣的点
specific_x_full = np.linspace(0, end, end*100)
specific_x_subset = np.linspace(0, end, end*100)
...
for beta in beta_values:
    ...
    function_values, grads1, x = train_model(model, criterion, optimizer, inputs, beta, epochs=2000,
                                             save_path=save_path)
...
# plt.figure(figsize=(10, 6))
# for beta, color in zip(beta_values, colors):
#     # 不再计算 Runge-Kutta 解，不再插入真值曲线
#     min_length = len(results[beta]["x"])
#     plt.plot(results[beta]["x"][:min_length], results[beta]["function_values"][:min_length],
#              label=f'PIDNs β={beta}', color=color, linestyle='--')
#
# plt.xlabel('x')
# plt.ylabel('f(x)')
# plt.title('Comparison of PIDNs for Different Beta (Full Range)')
# plt.legend()
# plt.savefig('comparison_plot_function_full.png')
# plt.show()
#
...
#     # 同样删除真值曲线相关代码
#     min_length = len(results[beta]["x"])
#     # 将结果限制在 [0, 10] 内，这里假设 results 的数据区间 >= 10
#     # 如果需要子集，可以通过索引限制结果范围
#     subset_mask = (results[beta]["x"] <= 10)
#     x_subset = results[beta]["x"][subset_mask]
#     function_values_subset = results[beta]["function_values"][subset_mask]
#
#     plt.plot(x_subset, function_values_subset, label=f'PIDNs β={beta}', color=color, linestyle='--')
#
# plt.xlabel('x')
# plt.ylabel('f(x)')
# plt.title('Comparison of PIDNs for Different Beta (0 to 10)')
# plt.legend()
...
#              label=f'Predicted Derivative (NN) β={beta}', color=color, linestyle='--')
#
# plt.xlabel('x')
# plt.ylabel('f\'(x)')
# plt.title('Comparison of Predicted Derivative for Different Beta (Full Range)')
# plt.legend()
...
#     subset_mask = (results[beta]["x"] <= 10)
#     x_subset = results[beta]["x"][subset_mask]
#     grads1_subset = results[beta]["grads1"][subset_mask]
#
#     plt.plot(x_subset, grads1_subset,
#              label=f'Predicted Derivative (NN) β={beta}', color=color, linestyle='--')
#
# plt.xlabel('x')
# plt.ylabel('f\'(x)')
# plt.title('Comparison of Predicted Derivative for Different Beta (0 to 10)')
# plt.legend()
